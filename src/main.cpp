...
#if defined(NDEBUG)
# error "Bitmea cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6799bf05c45f7644849c5114b976836b919170f174ae50234035b9685bc3da89");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitmea: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitmea Signed Message:\n";
...
{
    // Bitmea: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitmea: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitmea: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1508601016, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Past US Presidents take aim at Trump";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 250 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040990270abf01164b399cb290b2950bc6942df5a41e3a332b9147dc762bc8264ea7fe83a95420c268eb73a7c9928eb01be4edc8cdec84301d6030b1eb12d9fa57") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508601016;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1102123;
        ...
        assert(block.hashMerkleRoot == uint256("0x814da8f7cd41fed2b5415d9e2509444a744e016131a5244f135941f2b713042f"));
...
unsigned char pchMessageStart[4] = { 0xf9, 0x0a, 0x3b, 0x00 }; // Bitmea: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitmeaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitmeaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitmeaMiner(CWallet *pwallet)
{
    printf("BitmeaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitmea-miner");
    ...
    try { loop {
        ...
        printf("Running BitmeaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitmeaMiner, pwallet));
}
